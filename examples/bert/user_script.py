...
def bert_dataset(data_name: str, split: str, language: str, token: Union[bool, str] = True):
    return BertDataset(data_name).get_eval_dataset()


@Registry.register_dataloader()
def bert_dataloader(dataset, batch_size, *args, **kwargs):
...
def eval_accuracy(model: OliveModelHandler, device, execution_providers, data_dir, batch_size):
    dataloader = bert_dataloader(data_dir, batch_size)
    preds = []
    target = []
